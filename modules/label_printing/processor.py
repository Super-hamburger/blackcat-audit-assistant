...
class PdfOnlyLabelPrintProcessor:
    ...
    @staticmethod
    def _problem(pdf_path, page_index, label_type, reason, candidates=""):
        return {
            "pdf_file": pdf_path.name,
            "page": page_index + 1,
            "label_type": label_type,
        ...
        for pdf_path, document in documents.items():
            for page_index, pdf_page in enumerate(document):
                ...
                try:
                    ...
                    problems.append(
                        self._problem(pdf_path, page_index, label_type, str(error))
                    )
                else:
                    if marker is None:
                        excluded_pages += 1
    ...
    def run(self, progress_callback=None):
        task_dir = LabelPrintProcessor._task_output_dir(self.output_dir)
        documents = {}
        created_paths = []
        try:
            report_progress(
            ...
            try:
                ...
                LabelPrintProcessor._write_exception_report(task_dir, [{
                    "pdf_file": "",
                    "page": "",
                    "label_type": "",
                    "candidates": "",
            ...
            if problems:
                LabelPrintProcessor._write_exception_report(task_dir, problems)
                raise LabelPrintingError(problems[0]["reason"])
            ...
            LabelPrintProcessor._remove_created_files(created_paths)
            raise
        finally:
            for document in documents.values():
                document.close()
